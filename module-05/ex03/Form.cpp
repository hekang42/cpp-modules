...
Form::Form(std::string const &target, std::string const &name, int to_excute_grade, int to_signed_grade) : _target(target), _name(name), _to_sign_grade(to_signed_grade), _to_excute_grade(to_excute_grade)
{
	this->_is_signed = false;
}
...
Form::Form(const Form &other) : _target(other.getTarget()), _name(other.getName()), _to_sign_grade(other.getSignGrade()), _to_excute_grade(other.getExcuteGrade())
{
